...
// import { midAreaClickHandler } from "../utils/midareaClickHandler";
...
const MidArea = forwardRef(
  (
    ...
    useImperativeHandle(ref, () => ({
      flagClickHandler() {
        // midAreaClickHandler();
        for (let i = 0; i < flagClick.length; i++) {
          let parentTS =
            flagClick[i]?.parentNode.getAttribute("data-parent-ts");
          let getIds = document.querySelectorAll(
          ...
          console.log("hurrayyyy", flagClick?.parentNode, parentTS);
        }
      },
    }));
